# Sling Ring — Project Guide (CLAUDE.md)
...
```
SlingRing.Data        (rootns Data; no project refs — pure data, UnityEngine/System only)
   ▲
SlingRing.Core        (rootns Architecture; refs Data, InputSystem, UGUI)
   ▲
SlingRing.Game        (refs Data, Core, Zenject, DOTween, Cinemachine)
   ▲
SlingRing.UI          (refs Data, Core, Game, DOTween, TMP, InputSystem, UGUI)
   ▲
SlingRing.Bootstrap   (refs Data, Core, Game, UI, Zenject, InputSystem, UGUI) — the composition root, the ONLY assembly that sees everything
```
...
- Sub-configs: `PlayerConfig`, `EnemyConfig`, `CombatConfig`, `LevelConfig`, `PresentationConfig` (menuName `Sling Ring/Configs/...`).
...
Authoritative sources: member ordering = `Sling Ring.sln.DotSettings` (Rider `CSharpFileLayoutPatterns`, applied via **Rider Cleanup**); access modifiers = `.editorconfig` (`IDE0040` warning). **Run Rider Cleanup so a diff conforms.**
